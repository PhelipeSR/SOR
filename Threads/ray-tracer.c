...
void generateScene();
void *threads_trabalhadores(void);
...
int main(int argc, char ** argv)
{
 ...
 //   float rcp_samples;// = 1.0 / (float)samples;
	...
	int t;
	t = pthread_create(&thread, NULL, thread, );*/
	generateScene();
    ...
    //---ray tracing loop---
	//pthread_t thread;
	pthread_t trabalhadores[MAX_THREADS];
	//unsigned int t;
	//t = pthread_create(&thread, NULL, thread_geradora, (void*) altura);
	long b;
	int papa;
	for(b = 0; b < MAX_THREADS; b++)
	{
	papa = pthread_create(&trabalhadores[b], NULL, threads_trabalhadores,(void*) b);
		if(papa){
		printf("Deu merda");
		}
	pthread_join(trabalhadores[b], NULL);
	}
	pthread_exit(NULL);
...
void *threads_trabalhadores(void *blocksize){
    ...
    camera c;
    uchar *image;
    long bloco = (long) blocksize;
    long parcial = altura/blocksize;
    float rcp_samples;
    int samples;
    samples = 8;
    amostra = 0;    
    rcp_samples = 1.0 / (float)samples;
    for(parcial = 0 ; parcial < c.view.width ; parcial++)
    {
        for(profundidade = 0 ; profundidade < c.view.height ; profundidade++)
        {
            float r, g, b;
            r = g = b = 0.0;

            for(amostra=0; amostra < samples; amostra++) {
                ray rr = get_primary_ray(&c,altura,profundidade,amostra);    
            ...
            image[ 3* (altura * c.view.height + profundidade) + 0] = floatToIntColor(r);
            image[ 3* (altura * c.view.height + profundidade) + 1] = floatToIntColor(g);
            image[ 3* (altura * c.view.height + profundidade) + 2] = floatToIntColor(b);
        }
    }
	pthread_exit(NULL);
}
