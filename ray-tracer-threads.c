...
int main(int argc, char ** argv)
{
 ...
 //   float rcp_samples;// = 1.0 / (float)samples;
    ...
    //---ray tracing loop---
	pthread_t trabalhadores[MAX_THREADS]; //array contendo o numero maximo de threads definido ao início do programa
	long b = 0; 
	int thread_trabalhadora; //thread que vai executar o traçado de raios
	long razao = WID/MAX_THREADS; //Quantidade de pixels que cada thread deve processar
	for(b = 0; b < MAX_THREADS; b++)
	{
	long divisao = (b+1)*razao; //vai ser enviado para a subrotina da thread a quantidade de pixels que vai ser processada, e a quantidade de pixels que já foi processada
	...
	{
		pthread_join(trabalhadores[k], NULL); //desacopla as threads que estavam em execução
   ...
   free(rays);*/
    free(image);
    pthread_exit(NULL); //termina a thread pai
    //---exit---
    return 0;
}
...
void *threads_trabalhadores(void *blocksize){
    ...
    long escalonador = parcial - (WID/MAX_THREADS); //representa a quantidade de pixels já foi processada, pois depende do parâmetro que foi passado do main()
    int samples; 
    int j, s;
    samples = 8;
